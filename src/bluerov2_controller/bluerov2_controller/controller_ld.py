...
class Controller(Node):
    ...
    def __init__(self):
        ...
        # Subscribers for ArUco-follow mode
        ...
        self.control_mode_sub = self.create_subscription(
            String, "/settings/control_mode", self.control_mode_callback,  10
        )
    ...
    def control_mode_callback(self, msg):
        if msg.data in ["manual", "aruco"]:
            self.control_mode = msg.data
            self.get_logger().info(f"Control mode updated to {self.control_mode}")
        else:
            self.get_logger().warn(f"Invalid control mode: {msg.data}")
    ...
    def rc_callback(self, msg, topic):
        """Update RC channel values based on mode."""
        if self.control_mode == "manual":
            ...
            match topic:
                case 3: self.throttle        = msg.data
                case 13: self.yaw            = msg.data
                case 11: self.forward        = msg.data
                case 12: self.lateral        = msg.data
                case 8: self.camera_tilt     = msg.data
                case 9: self.lights          = msg.data
                case 10: self.gripper        = msg.data
