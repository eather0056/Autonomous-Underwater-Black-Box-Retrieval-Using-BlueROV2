...
class Controller(Node):
    ...
    def __init__(self):
        ...
        self.pitch              = 1500                              # The pitch channel --got ignored (RC1) refers to the upward or downward tilt of the BlueRov2 HEAVY.            
        ...
        self.camera_pan         = 1500                              # The Camera Pan channel --got ignored (RC7) controls the horizontal panning movement of the camera.
        self.camera_tilt        = 1500                              # The Camera Tilt channel (RC8) controls the vertical tilt movement of the camera.
        self.lights             = 1500                              # The Lights Level channel (RC7) controls the intensity or brightness of the light source.
        self.gripper            = 1500                              # The pitch channel (RC1) refers to the upward or downward tilt of the BlueRov2 HEAVY.            
        ...
        # Desired setpoints for ArUco-follow mode
        self.target_depth = 4.5  # meters
        ...
        self.rc_gripper_sub     = self.create_subscription(UInt16, "/bluerov2/rc/gripper", lambda msg: self.rc_callback(msg, 9), 10)
        # Subscriber to ArUco pose and control mode
        self.pose_sub           = self.create_subscription(PoseStamped, '/aruco_pose', self.aruco_pose_callback, 10)
        self.mode_sub           = self.create_subscription(String, "/settings/control_mode", self.control_mode_callback, 10)
        ...
        self.depth_set_pub      = self.create_publisher(Float64, "/settings/depth/set_depth", 10)
    ...
    def send_bluerov_commands(self):
        ...
        rc_channel_values = (self.gripper,
                             1500,
                             self.throttle,
                             self.yaw,
                             self.forward,
    ...
    def control_mode_callback(self, msg):
        ...
        self.get_logger().info(f"Control mode updated to {self.control_mode}")

    def aruco_pose_callback(self, msg):
        self.aruco_pose = msg.pose

    def update_aruco_control(self):
        ...
        y = self.aruco_pose.position.y
        z = self.aruco_pose.position.z

        # Control logic
        depth_command = Float64()
        depth_command.data = self.target_depth
        self.depth_set_pub.publish(depth_command)

        # Calculate forward/lateral alignment
        forward_pwm = self.calculate_pwm(x, self.target_distance)
        lateral_pwm = self.calculate_pwm(y, 0)

        forward_msg = UInt16()
        lateral_msg = UInt16()
        forward_msg.data = forward_pwm
        lateral_msg.data = lateral_pwm

        self.forward_pub.publish(forward_msg)
        self.lateral_pub.publish(lateral_msg)

    def calculate_pwm(self, current, target):
        error = current - target
        pwm = self.pwm_neutral + int(self.KP * error)
        return pid.saturation(pwm, self.pwm_neutral, self.pwm_max)

    def send_bluerov_commands(self):
        if self.control_mode == "manual":
            super().send_bluerov_commands()
        elif self.control_mode == "aruco":
            self.update_aruco_control()
